...
# for those genes that can't find inter-accession homologous, we ignore them in the final homology file


def find_self_file(file):
    nam = os.path.basename(file).strip("_out_clean")
    nam += "_*.fasta"
    file_ls = glob.glob(os.path.join(path, nam))
    return len(file_ls) == 1  # there is only one .fasta file for the self_blast input
...
def insert_hog(f_1, hogg):  # df is the output of remove_subset_list
    output = []
    numb = 0
    for x in f_1:
...
def simplify_self_result(dc, hog):  # df is a dataframe read from the self result
    ...
    for gen in qgene:  # both can and col level
            ...
            if len(q_sseq) == 0:  # could be can&col only for can query, and all the tair level for col query
                ...
                for g in q_sseq_sseq:
                    if ((qgene_arr['qseqid'] == gen) & (qgene_arr['sseqid'] == g)).any():  # can and tair match in qgene
                        ...
                        pass
    return output  # '\n'.join('\t'.join(sub) for sub in output)
...
for f in blast_files:
    hog = os.path.basename(str(f)).strip("_out_clean")
    name = hog + "_homology"
