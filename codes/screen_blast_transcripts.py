# this is simply copy the screen_blast_duplicated_gene.py for duplicated genes
...
def find_self_file(f):
    return "recip" in f  # file a_f is a self_blast file
    ...
    if os.stat(f).st_size == 0:
        ...
        with open(f, 'r') as a_f:
            df = pd.read_csv(a_f, sep='\t',
                             names=["qseqid", "sseqid", "evalue", "bitscore", "length", "pident", "qstart", "qend", "qlen", "sstart", "send", "slen"])
            gene_lst = df['qseqid'].unique()  # select query genes
            if find_self_file(name):  # if is a self_blast result
